...
definir_func: funcion (variable asignar)? identificador argumentos saltolinea+ instruccion* finfuncion saltolinea*;
...
para_op:
	((para variable asignar expr (desde expr)? hasta expr (conpaso expr)?) | (paracada identificador de variable)) hacer? saltolinea+ instruccion* finpara puntocoma? saltolinea+;
